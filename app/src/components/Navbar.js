...
const Navbar = ({list}) =>  {
  ...
  return (
    <div  className="navbar">
      ...
      <div className="icons">
      <BsListCheck onClick={() => handleModal("Tareas Completadas", "completed")} />
      <MdDelete onClick={() => handleModal("Tareas Eliminadas", "deleted")}/>
      </div>
      </div>
      <img src={logo} alt={logo}/>
      <p>El éxito es la suma de pequeños esfuerzos repetidos un día sí y otro también. </p>
      {openModal? <Modal setOpenModal={setOpenModal} title={modalInfo.title} status={modalInfo.status} list={list}/>:""}
    </div>
  )
}
